...
class Grid2dNumba:
    def __init__(self, grid, radius=8):
        ...
        indices = np.meshgrid(np.arange(self.voxel_grid_2d.shape[0]), np.arange(self.voxel_grid_2d.shape[1]), indexing='ij')
        ...
        # 确保使用正确的 indices 变量
        self.indices = np.array([(x, y, angle) for x, y in self.area_indices for angle in theta])
    ...
    def get_area_coverage(self):
        return compute_area_coverage(self.voxel_indices, self.hit_angles, self.coord_range, self.grid_range, self.indices, self.fov, self.grid_size)
